    ...
    def __init__(
        ...
        **kwargs
        ...
        if lora_config['rank'] == 0:
            ...
            self.reg_query_proj = LoRALinear(self.embed_dim, self.embed_dim, bias=query_proj_bias, lora_rank=lora_config['rank'], lora_alpha=lora_config['alpha'], lora_dropout=lora_config['dropout'])
            self.reg_key_proj = LoRALinear(self.embed_dim, self.embed_dim, bias=key_proj_bias, lora_rank=lora_config['rank'], lora_alpha=lora_config['alpha'], lora_dropout=lora_config['dropout'])
            self.reg_value_proj = LoRALinear(self.embed_dim, self.embed_dim, bias=value_proj_bias, lora_rank=lora_config['rank'], lora_alpha=lora_config['alpha'], lora_dropout=lora_config['dropout'])
            self.reg_out_proj = LoRALinear(self.embed_dim, self.embed_dim, bias=out_proj_bias, lora_rank=lora_config['rank'], lora_alpha=lora_config['alpha'], lora_dropout=lora_config['dropout'])
        ...
        if self.num_summary > 0:
            if share_query_proj:
                ...
                if lora_config['rank'] == 0:
                    ...
                    self.sum_query_proj = LoRALinear(self.embed_dim, self.embed_dim, bias=query_proj_bias, lora_rank=lora_config['rank'], lora_alpha=lora_config['alpha'], lora_dropout=lora_config['dropout'])
            ...
            if share_key_proj:
                ...
                if lora_config['rank'] == 0:
                    ...
                    self.sum_key_proj = LoRALinear(self.embed_dim, self.embed_dim, bias=key_proj_bias, lora_rank=lora_config['rank'], lora_alpha=lora_config['alpha'], lora_dropout=lora_config['dropout'])
            ...
            if share_value_proj:
                ...
                if lora_config['rank'] == 0:
                    ...
                    self.sum_value_proj = LoRALinear(self.embed_dim, self.embed_dim, bias=value_proj_bias, lora_rank=lora_config['rank'], lora_alpha=lora_config['alpha'], lora_dropout=lora_config['dropout'])
            ...
            if not self.no_sum_out:
                if share_out_proj:
                    ...
                    if lora_config['rank'] == 0:
                        ...
                        self.sum_out_proj = LoRALinear(self.embed_dim, self.embed_dim, bias=out_proj_bias, lora_rank=lora_config['rank'], lora_alpha=lora_config['alpha'], lora_dropout=lora_config['dropout'])
            ...
            if share_key2_value2_proj_weight:
                if lora_config['rank'] == 0:
                    ...
                    self.sum_key2_proj = LoRALinear(self.embed_dim, self.embed_dim, bias=False, lora_rank=lora_config['rank'], lora_alpha=lora_config['alpha'], lora_dropout=lora_config['dropout'])
                ...
                if lora_config['rank'] == 0:
                    ...
                    self.sum_key2_proj = LoRALinear(self.embed_dim, self.embed_dim, bias=sum_key2_proj_bias, lora_rank=lora_config['rank'], lora_alpha=lora_config['alpha'], lora_dropout=lora_config['dropout'])
                    self.sum_value2_proj =  LoRALinear(self.embed_dim, self.embed_dim, bias=sum_value2_proj_bias, lora_rank=lora_config['rank'], lora_alpha=lora_config['alpha'], lora_dropout=lora_config['dropout'])
